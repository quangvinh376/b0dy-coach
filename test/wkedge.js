...
(async function(){
  ...
  await run('W0', 'Tắt dải → WebKit gặp body position:fixed phủ kín viewport = "viewport-sized" (giữ màu cũ) — bẫy của v2.4.0–2.4.1', async function(){
    ...
    check(bt.dt==='block' && bt.top==='95px' && bt.bot==='28px' && bt.busy==='93px' && bt.pill==='99px' && bt.sheet==='28px', 'bản cài black-translucent (inset 59): dải bật, khối đỉnh 95 (dưới dải kính 59+35), đệm đáy 28, pill 99, chân sheet 28 — '+JSON.stringify(bt));
