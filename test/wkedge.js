...
(async function(){
  ...
  await run('W0', 'Tắt dải → WebKit gặp body position:fixed phủ kín viewport = "viewport-sized" (giữ màu cũ) — bẫy của v2.4.0–2.4.1', async function(){
    ...
    /* bản cài cũ (standalone + sb-legacy, vùng web dưới thanh, --top 2px): dải tắt, khối đỉnh và vạch .busy không bị che (sự cố v2.4.2);
       bản cài black-translucent (standalone, inset 59): dải bật; né dải kính (59+35) ở đỉnh và vùng không vẽ (59) ở đáy */
    var bt=await ev(function(){ var d=document.documentElement; d.classList.add('standalone'); d.style.setProperty('--sat','59px'); var r={dt:getComputedStyle(document.getElementById('wk-t')).display, top:getComputedStyle(document.getElementById('p-pin')).paddingTop, bot:getComputedStyle(document.getElementById('p-pin')).paddingBottom, busy:getComputedStyle(document.getElementById('busy')).top, pill:getComputedStyle(document.getElementById('pill')).top, sheet:getComputedStyle(document.getElementById('lib')).paddingBottom}; d.classList.remove('standalone'); d.style.removeProperty('--sat'); return r; });
    check(bt.dt==='block' && bt.top==='95px' && bt.bot==='87px' && bt.busy==='93px' && bt.pill==='99px' && bt.sheet==='87px', 'bản cài black-translucent (inset 59): dải bật, khối đỉnh 95 (dưới dải kính 59+35), đệm đáy 87 (trên vùng không vẽ 59), pill 99, chân sheet 87 — '+JSON.stringify(bt));
